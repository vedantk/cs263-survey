...
encode type information, structure connectivity properties, and ordering
properties. In addition, the authors implemented a domain-specific language
for `programming' these invariants. An analyzer-generator was implemented to
transform these invariants into a program that enforces them. These results
are significant for their general nature and emphasis on practicality.
...
or if there are techniques that minimize this loss for certain classe of
...
time and space complexity concerns mentioned earlier. A final non-trivial
hurdle is the difficulty of implementing these techniques for complex,
