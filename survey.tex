...
execution), but with a simplified operational semantics. The key idea is to
replace concrete program values with abstract values from a lattice, and to
then simulate the resulting program until a fixed-point criterion is met.

A good early exposition of abstract interpretation is found in [JM82]. It
begins with an explanation of the use of lattices to approximate concrete
program values. The authors specify a concrete-value lattice $A$ and an
abstract approximation lattice $A'$, and define a pair of functions which
translate values between the two lattices: $abs : A \rightarrow A'$ and
$conc : A' \rightarrow A$. Then they state an important \textit{safe
...
version of the abstract operation. While this is a general criterion, the
rules for abstracting the \texttt{cons} operation in [JM82] implicitly rely
on this property.

The goal of the abstract interpretation in [JM82] is to find the subset $S
\subseteq \Sigma$ of finitely reachable states
In order to construct their data-flow analyzer, 
...
structures, perform interprocedural analysis, and easily trade speed for
precision by adjusting its token sets.
